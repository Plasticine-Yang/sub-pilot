...
- 后端：Rust（转写走 whisper.cpp / whisper-rs，媒体处理走 ffmpeg）
...
# 2. 拉取随包二进制资源（ffmpeg 静态二进制 + Whisper base 模型）
...
首次启动时应用会自检 `resources/ffmpeg/ffmpeg` 可执行、`resources/models/ggml-base.bin` 存在；缺失时首页会给出明确提示。
